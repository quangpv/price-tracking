...
import { ChartSkeleton, TableSkeleton } from '../../components/Shimmer/Shimmer';
...
export const GoldPage: React.FC<GoldPageProps> = ({ currency }) => {
  ...
  const { vnGoldData, worldGoldPrice, chartData, loading, lastPolled } = useGold(currency);
             ...
             {worldGoldPrice ? (
              <div className="space-y-3 animate-fade-in">
                <p className="text-3xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">
                  {formatPrice(worldGoldPrice.currentPrice, 'usd')}/oz
                </p>
                {currency === 'vnd' && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    ≈ {formatPrice(worldGoldPrice.currentPrice * 24850, 'vnd')}/oz
                  </p>
                )}
                {lastPolled > 0 && (
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    {t('goldPage.lastUpdated')}{formatTimestamp(lastPolled / 1000)}
                  </p>
                )}
              </div>
            ) : null}
        ...
        <Card className="overflow-hidden">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-2">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">
              {t('goldPage.lastUpdated')}{formatTimestamp(vnGoldData.lastUpdated)}
            </span>
          </div>
