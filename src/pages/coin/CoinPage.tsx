...
import {useCoinState} from './hooks/useCoinState.ts';
import {LineChartComponent} from '../../shared/components/LineChart/LineChart';
import {ChartSkeleton, StatCardSkeleton} from '../../shared/components/Shimmer/Shimmer';
...
export const CoinPage: React.FC<CoinPageProps> = ({currency}) => {
    ...
    const {
        currentPrice, chartData, loading, error
    } = useCoinState(currency);
            ...
            {currentPrice && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
                    <StatCard
                        ...
                        sublabel={`${t('coinPage.marketCap')}${currentPrice.marketCap}`}
                    />
                </div>
            )}
